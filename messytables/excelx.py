...
class XLSXRowSet(RowSet):
    ...
    def raw(self, sample=False):
        ...
        num_rows = min(self.window, self.sheet.max_row) if sample else self.sheet.max_row
        for row in self.sheet.iter_rows(min_row=None, max_row=num_rows):
            row_list = []
            for cell in row:
                value = cell.value
                if not isinstance(cell, openpyxl.cell.MergedCell) and cell.is_date:
                    cell_type = DateType(None)
                elif cell.data_type == 'n':
                    cell_type = IntegerType()
                else:
                    cell_type = StringType()
                row_list.append(Cell(value, type=cell_type))
            yield row_list
